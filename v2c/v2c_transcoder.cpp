...
namespace can
{
	...
	static sig_asm make_sig_agg(const tr_signal &sig)
	{
		...
		{
		case i64:
			return sig_asm(sig_agg<int64_t>(sig));
		case u64:
			return sig_asm(sig_agg<uint64_t>(sig));
		case f32:
	...
	{
		auto sig_it = std::find_if(_signals.begin(), _signals.end(), [&](const auto &s)
	...
	{
		using namespace std::chrono;
		// std::cout << "Inside transcode";
		setup_timers(stamp);
		_update_freq = milliseconds(100);
